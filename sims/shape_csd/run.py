#!/usr/bin/env python3
"""shape_csd — does probe-recovery time warn before the shape snaps?

Retrofit of original_shape_csd_probes.py to Sim Harness Standard v1 (§5 item 2,
"the headline CSD claim"). Reads ONLY config.json + CLI overrides (§3).

Stdlib only, like the original — this one needs no numpy.

Changes from the original, recorded rather than silent:
  - a monostable-strut null arm at every point. The original measured the
    bistable arm alone, which cannot separate critical slowing down from a
    loaded structure simply getting softer.
  - the verdict is decided on the between-arm ratio crossing a pre-committed
    1.5x threshold once, not on the bistable arm's curve or on a scan.
  - probe magnitude swept {0.03, 0.05, 0.08}; the original used 0.05.
  - 5 seeds. The original was deterministic apart from a fixed random.Random(7)
    in the variance section; seeds here perturb the probe direction so the
    recovery estimate is not a single privileged kick.
  - 12 compressions instead of 8, for finer lead resolution.
  - fluctuation variance kept as a descriptive secondary, no threshold attached.
"""
...
def strut_force(length: float, m: dict[str, Any], bistable: bool) -> float:
    ...
    return -m["monostable_stiffness"] * (length - 0.5 * (l1 + l2))
...
def recovery_time(comp: float, m: dict[str, Any], bistable: bool,
                  magnitude: float, rng: random.Random) -> float:
    ...
    for t in range(m["recovery_max_t"] // step):
        ...
        if abs(strut_len(Vk) - l0) < m["recovery_tol"] * dev0:
            return float(t * step)
    return float(m["recovery_max_t"])          # censored: probe never returned
...
def _curve(seed: int, magnitude: float, config: dict[str, Any],
           bistable: bool) -> tuple[list[float], list[float]]:
    m = config["mechanics"]
    ...
    taus = [recovery_time(c, m, bistable, magnitude, rng) for c in comps]
    return comps, taus
...
def measure(seed: int, sweep: dict[str, Any], config: dict[str, Any]) -> dict[str, float]:
    ...
    snap = _snap_for(config)
    comps, tau_bi = _curve(seed, magnitude, config, bistable=True)
    _, tau_mono = _curve(seed, magnitude, config, bistable=False)
...
def null(seed: int, sweep: dict[str, Any], config: dict[str, Any]) -> dict[str, float]:
    """monostable_strut — the same protocol on a frame with no fold."""
    magnitude = sweep["probe_magnitude"]
    snap = _snap_for(config)
    comps, tau_mono = _curve(seed, magnitude, config, bistable=False)
